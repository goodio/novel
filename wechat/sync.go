...
func (wechat *WeChat) beginSync() error {
	...
	logrus.Info(`查找同步服务器，发现同步服务器后可以开始接收消息 ...`)

	didGetSyncHost := wechat.choseAvalibleSyncHost()

	if !didGetSyncHost {
		return errors.New("未能匹配同步服务器, 请登录重试...")
	}

	logrus.Infof(`发现同步服务器 [%s], 开始同步 ... ...`, wechat.syncHost)
		...
		if selector == `0` {
			...
			for continueFlag != 0 {
				...
				if selector == `2` && resp.AddMsgCount > 0 {
					logrus.Infof(`收到 %d 条消息...`, resp.AddMsgCount)
				}
...
func (wechat *WeChat) choseAvalibleSyncHost() bool {
	...
	for _, host := range hosts {
		logrus.Infof("尝试连接: %s ... ... ", host)
		wechat.syncHost = host
		code, _, _ := wechat.syncCheck()
		if code == `0` {
			return true
		}
		logrus.Warnf("%s 连接失败", host)
	}
