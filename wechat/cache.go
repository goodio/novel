...
func (c *cache) updateContact(v map[string]interface{}) {
	nc, err := newContact(v)
	if err != nil {
		logrus.Errorf(`create contact failed error: %v`, err)
	} else {
		if len(nc.UserName) > 0 {
			oc := c.contacts[nc.NickName]
			if oc != nil {
				//logrus.Info(`old contact: %v will be replaced by %v`, oc, nc)
			}
			c.contacts[nc.UserName] = nc
		} else {
			logrus.Warningf(`bad contact %v`, v)
		}
	}
}
